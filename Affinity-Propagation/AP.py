...
if __name__ == "__main__":
    ...
    for i, mul in enumerate(np.linspace(1, 4, 9)):
        ...
        print ('p = %.1f' % mul), p, '#cluster：', n_clusters
        y_hat = af.labels_

        plt.subplot(3, 3, i+1)
        plt.title(u'Preference：%.2f，#cluster：%d' % (p, n_clusters))
        clrs = []
        ...
        for k, clr in enumerate(clrs):
            cur = (y_hat == k)
            plt.scatter(data[cur, 0], data[cur, 1], c=clr, edgecolors='none',alpha=0.8)
        ...
        plt.grid(True)
    plt.tight_layout()
    plt.suptitle(u'Affinity Propagation', fontsize=20)
